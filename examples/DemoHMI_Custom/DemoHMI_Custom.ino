...
//---- AB&T EasyCAT shield simple HMI custom application example ----------------------------------------------------  


// This is the legacy "DemoHMI" example but the variables have been 
...
//
//    int16_t    Analog_X                 The first analog input         
//    int16_t    Analog_Y                 The second analog input
//
// And the output:
//
//    uint8_t     Leds;                   The four leds displayed on the TFT
//    uint8_t     RisigTestRamp;          A test counter


//*********************************************************************************************

#define CUSTOM                      // Custom mode
#include "DemoHMI_Custom.h"         // This file has been created by the Easy Configurator 
                                    // and must be located in the Arduino project folder
                                    //
                                    // There are two others files created by the Easy Configurator:
                                    // DemoHMI_Custom.bin that must be loaded into the EEPROM.
                                    // DemoHMI_Custom.xml that must be used by the EtherCAT master. 
...
EasyCAT EASYCAT(8);         // Instantiate the EasyCAT library
...
//                             -------------------------------- output data management ---------------------------------------------
    ...
    Analog_Y =  EASYCAT.BufferOut.Cust.Analog_Y;                          // read the analog values from the variables "Analog_X" and "Analog_Y"
    Analog_X =  EASYCAT.BufferOut.Cust.Analog_X;                          // i.e. from the data that come from the Master
                                                                      
    Analog_Y = Analog_Y >> 3;                                             // normalize the values on 12 bit i.e. from 0 t0 4095
    Analog_X = Analog_X >> 3;                                             //
...
//                             -------------------------------- input data management ----------------------------------------------
    ...
    EASYCAT.BufferIn.Cust.Leds = LedStatus;                               // The status of the leds is mapped to "Leds"
    
    EASYCAT.BufferIn.Cust.RisingTestRamp = Conta++;                       // We also increase Conta every cycle and put it in 
                                                                          // "RisingTestRamp", just for test pourpose  
...
//----- print a value between -4095 and 4095, right justified  ------------------------------------------------------------
